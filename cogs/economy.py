...
class Economy(commands.Cog):
    ...
    async def sell(self, interaction: discord.Interaction, shark: str, amount: int = 1):
        ...
        if shark not in SHARKS:
            await interaction.followup.send(f"Unknown shark: **{shark}**", ephemeral=True)
            return
        if amount < 1:
        ...
        if not row or row["count"] < amount:
            ...
            await interaction.followup.send(
                f"You only have **{have}** {shark} Shark(s).", ephemeral=True
            )
            return
        ...
        emoji = get_emoji(shark, interaction.guild)
        embed = discord.Embed(
            title="💰 Sharks Sold!",
            description=f"Sold **{amount}x {shark} Shark** {emoji}\nEarned: **{value:,} Shark Dollars**",
            color=0xf1c40f,
        )
        await interaction.followup.send(embed=embed)
    ...
    async def pay(self, interaction: discord.Interaction, member: discord.Member, amount: int):
        ...
        if row["shark_dollars"] < amount:
            await interaction.followup.send(
                f"You only have **{row['shark_dollars']:,} SD**.", ephemeral=True
            )
            return
        await self.bot.db.execute(
            "UPDATE users SET shark_dollars=shark_dollars-$1 WHERE user_id=$2",
